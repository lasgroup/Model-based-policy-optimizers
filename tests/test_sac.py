...
def policy(x):
    return optimizer.act(x, sac_output.sac_state, sac_output.sac_state.system_params, evaluate=False)


def step(x, _):
    u = policy(x)[0]
    next_sys_state = system.step(x, u, sac_output.sac_state.system_params)
...
def test_sac_good_fit():
    assert sac_output.sac_summary[-1]['eval/episode_reward'] >= -400
